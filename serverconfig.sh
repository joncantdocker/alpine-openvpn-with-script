...
docker run -v $OVPN_DATA:/etc/openvpn --rm -it joncantdocker/alpine-openvpn-with-script initpki
...
docker run -v $OVPN_DATA:/etc/openvpn --rm joncantdocker/alpine-openvpn-with-script listcerts
...
do
	...
	docker run -v $OVPN_DATA:/etc/openvpn --rm -it joncantdocker/alpine-openvpn-with-script easyrsa build-client-full $ClientName

	#save the certificate to a file called ClientName.ovpn
	docker run -v $OVPN_DATA:/etc/openvpn --rm joncantdocker/alpine-openvpn-with-script getclient $ClientName > $ClientName.ovpn
